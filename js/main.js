...
const ui = {
  ...
  renderSlack: document.getElementById('renderSlack'),
...
// Minimum wall-clock gap between renders = target slot minus the #renderSlack input (ms,
// default 4). The slack (≈ half a 120Hz tick) absorbs rAF jitter so a 120Hz display renders
// every OTHER tick cleanly (no beating), while a 60Hz display still renders every tick. Set
// it to 0 to gate at the full target slot. Upper bound only — NOT a v-sync lock.
function renderSlackMs() {
  const v = ui.renderSlack ? parseFloat(ui.renderSlack.value) : 4;
  return isFinite(v) && v >= 0 ? v : 4;
}
function minRenderMs() { return targetFrameMs() - renderSlackMs(); }
let lastRenderTs = 0;
