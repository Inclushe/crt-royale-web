...
so a 120Hz display doesn't run the full chain twice as often; the cap's early-render slack is the
`#renderSlack` input (ms, default 4 — `minRenderMs() = targetFrameMs() - renderSlackMs()`, 0 = gate
at the full slot). GPU time is the real "will it hold 60fps" cost. (The old `#frameTime` **CPU** submit
